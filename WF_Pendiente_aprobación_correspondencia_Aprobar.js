...
(async () => {
    ...
    await recorder.start('./Videos/WF_Aprobación_Correspondencia_Aprobar.mp4');
    await animate(page);
    await new Promise(r => setTimeout(r, 3000));
    //#endregion

    //#region Inicio de sesión
    await page.type('input[name="username"]', 'GESTIONDOCUMENTAL')
    await page.type('input[name="password"]', 'Uninavarra2023')

    await page.screenshot({ path: './Capturas/pagina_Login_Aprobación_Correspondencia_Aprobar.png' })
    await page.click('button.btn.btn-primary.loginButton')
    await new Promise(r => setTimeout(r, 20000));
    await page.screenshot({ path: './Capturas/bandeja_Aprobación_Correspondencia_Aprobar.png' })
    ...
    await ingresoDocuments.click('#tblAdHoc2290')
    console.log('Se da click en la tarea Aprobar')
    await new Promise(r => setTimeout(r, 6000));

    await ingresoUserInteract.click('input[name="OBBtn_Yes"]')
    console.log('Se da click en el botón Sí para finalizar el flujo')
    await new Promise(r => setTimeout(r, 6000));
    //#endregion
